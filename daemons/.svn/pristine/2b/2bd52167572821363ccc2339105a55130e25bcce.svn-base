...
{
	QFile confFile( "/etc/hostapd.conf" );
	QFile confDefaultFile( "/etc/hostapd.conf.default" );
...
void getInterfacesWLAN0Conf( bool* isDHCP, QString* address, QString* netmask, QString* mode )
{
	...
	{
		QProcess process;
		process.start( QString("sudo cat %1").arg( "/etc/NetworkManager/system-connections/wlan0" ) );
...
void getWpaSupplicantConf( QString* ssid, QString* proto, QString* psk )
{
	...
	QFile confDefaultFile( "/etc/wpa_supplicant/wpa_supplicant.conf.default" );
