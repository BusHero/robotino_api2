...
#ifdef WIN32
#define LOGFILE "grapplerd_log.txt"
#else
#define LOGFILE "/var/log/grapplerd"
#endif
...
void printHelp()
{
	std::cout << "grapplerd version " << BuildVerStr << std::endl;
	std::cout << "grapplerd [OPTIONS]" << std::endl;
	...
	std::cout << "-device=x       : open device at x (default /dev/grappler)" << std::endl;
...
int main( int argc, char** argv )
{
	...
	app.setApplicationName( "grapplerd" );
	...
	client->readParameters( "/etc/robotino/grapplerd.conf", "/home/robotino/.config/grapplerd.conf" );
