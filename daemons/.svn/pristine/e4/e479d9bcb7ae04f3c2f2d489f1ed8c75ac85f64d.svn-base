...
public:
	...
	void runRobotinoView( const QString& programPath );
	bool isRobotinoViewRunning( const QString& programPath ) const;
	void stopRobotinoView();
	
	QMap<QString,QString> globalVars() const;
	QStringList robviewOutput() const;
...
private:
	...
	void parseRobotinoViewOutput(const QByteArray& data);

	static ScriptRunner* _impl;
	
	const QString _robviewProgram;

	QMap< QString, QProcess* > _processMap;
	QMap< QString, Q_PID > _pidMap;
	
	mutable QMutex _globalVarsMutex;
	QMap<QString,QString> _globalVars;
	QStringList _robviewOutput;
	const int _robviewOutputMaxSize;
