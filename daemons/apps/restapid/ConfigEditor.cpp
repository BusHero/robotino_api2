...
bool ConfigEditor::write( const QString& fileName, ConfigRestartItem& restartItem )
{	
	QFile file( fileName );
	...
	{	
				...
				)
			{
				...
				else
				{
					...
					if( index > -1 )
					{
						...
						QString newValue = _values.value(key);

						if( _values.contains( key ) && newValue != value)
						{
							tempStream << name << sep << newValue << '\n';
							lineModified = true;

							items[key] = _values[key];
							
							qDebug() << fileName << ": " << key << " changed from " << value << " to " << newValue;
		...
		if( reader.items() == items )
		{
			...
			if(configChanged)
			{
				restartItem = reader.restartItem();
			}
