...
QString ConfigReader::value(const QString key, const QString defaultValue ) const
{
	return _items.value(key,defaultValue);
}

void ConfigReader::setValue(const QString key, const QString value)
{
	_items[key] = value;
}
			...
			if( line.length() >= 5 )
			{
				if( line.contains( QRegExp("^(#|;)@web") ) )
				{
					...
					if( l.count() > 1 )
					{
						...
						{
							rec::robotino::daemons::Log::singleton()->log( "fileselection", 0 );
							webItem.type = ConfigItem::ItemType_selection;
							QStringList fileList;
							if( l.count() > 2 )
							{
								QString path = l.at(2);
								rec::robotino::daemons::Log::singleton()->log( "path: " + path, 0 );
								QStringList filters;
								for( int i=3; i<l.count(); ++i )
								{
									filters << l.at(i);
									rec::robotino::daemons::Log::singleton()->log( "filter: " + l.at(i), 0 );
								}
								QDir dir( path );
							...
							for( int i=0; i<fileList.count(); ++i )
							{
								...
								rec::robotino::daemons::Log::singleton()->log( "fileList: " + fileList.at(i), 0 );
							}
						}
						else
						{
				...
				else
				{
					...
					if( index > -1 )
					{
						...
						if( isWebItemFound )
						{
							...
							//qDebug() << "webItemFound " << webItem.name;
							_webItems << webItem;
						}
					}
				}
