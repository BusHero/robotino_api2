...
, _isOverCurrentDetected( false )
{
	...
	connected &= connect( _overCurrentTimer, SIGNAL( timeout() ), SLOT( on_overCurrentTimer_timeout() ) );
	Q_ASSERT( connected );
}

void Motor::on_set_motor_mode_changed( const QString& mode )
{
	Mode_t newMode;
...
void Motor::set_motor_readings( const QVector< float >& speeds, const QVector< int >& positions, const QVector< float >& currents, float time_delta )
{
	...
	if( _current.mean() > _maxcurrent && _setPoint != 0.0f )
	{
...
void Motor::set_motor_setpoint( float speed, const rec::rpc::ClientInfo& )
{
	...
	case FreeMode:
		Q_EMIT log( QString( "motor %1 free mode" ).arg( _id+1 ), 3 );
		if( _isOverCurrentDetected )
		{
...
{
	QString str = client->getParameter( QString( "~/Motor%1/mode" ).arg( _id+1 ), "free" ).toString();
	if( "vel" == str )
	{
		_mode = VelMode;
	}
	...
	_maxcurrent = client->getParameter( QString( "~/Motor%1/maxcurrent" ).arg( _id ), 0.0 ).toDouble();
	_exceedtime = client->getParameter( QString( "~/Motor%1/exceedtime" ).arg( _id ), 0 ).toUInt();
