...
typedef enum { UnknownPhy,
       DP83848C, //from Keil MCB2300
       IP175D,   //from EA09
     } PhyType_t;

PhyType_t phyType = UnknownPhy;
...
static void waitForLinkGood( void );
static void configureDuplexMode( void );
...
void init_ethernet (void) {
   ...
   */
   ...
   else if( 0x243 == id1 && 0xD80 == id2 )
   {
     phyType = IP175D;
   }

   if( DP83848C == phyType || IP175D == phyType ) //IP175D
   {
...
static void configureDuplexMode( void )
{
  ...
  {
    regv = read_PHY( PHY_REG_BMSR );
	if( bitIsSet( regv, 14 ) ) //100Base-X full duplex Capable
	{
	  /* Full duplex is enabled. */
      MAC_MAC2    |= MAC2_FULL_DUP;
      MAC_COMMAND |= CR_FULL_DUP;
	...
	else if( bitIsSet( regv, 13 ) ) //100Base-X half duplex Capable
	{
	  /* Half duplex mode. */
      MAC_IPGT = IPGT_HALF_DUP;
	...
	else if( bitIsSet( regv, 12 ) )	//10Base-T full duplex Capable
	{
	  /* Full duplex is enabled. */
      MAC_MAC2    |= MAC2_FULL_DUP;
      MAC_COMMAND |= CR_FULL_DUP;
	...
	else if( bitIsSet( regv, 11 ) )	//10Base-T half duplex Capable
	{
	  /* Half duplex mode. */
      MAC_IPGT = IPGT_HALF_DUP;
