...
BYTE  Fat_RootDir[FAT_SIZE + ROOT_DIR_SIZE];
extern const unsigned char BootSect[];

BYTE  Memory[MSC_MAX_PACKET]; 
...
BOOL MSC_Reset (void) {
  ...
  FIO4CLR = (LED_RD | LED_WR);  /* Turn Off R/W LED */
...
void MSC_MemoryRead (void) {
  ...
  if (BulkStage != MSC_BS_DATA_IN) {
    FIO4CLR = LED_RD;            /* Turn Off Read LED */
...
void MSC_MemoryWrite (void) {
  ...
  if ((Length == 0) || (BulkStage == MSC_BS_CSW)) {
  	if ( startedWriting )
	{
	  ...
	  timer_afterWrite_start();
	}
    startedWriting = FALSE;
    FIO4CLR = LED_WR;            /* Turn Off Write LED */
    CSW.bStatus = CSW_CMD_PASSED;
    MSC_SetCSW();
  }
}
...
fail: CSW.bStatus = CSW_CMD_FAILED;
      ...
      switch (CBW.CB[0]) {
        ...
        case SCSI_READ10:
          if (MSC_RWSetup()) {
            if ((CBW.bmFlags & 0x80) != 0) {
              FIO4SET = LED_RD;    /* Turn On Read LED */
        ...
        case SCSI_WRITE10:
          if (MSC_RWSetup()) {
            if ((CBW.bmFlags & 0x80) == 0) {
			  startedWriting = FALSE;
              FIO4SET = LED_WR;    /* Turn On Write LED */
  ...
  firmware = (BYTE *)USER_FLASH_START;
    ...
    if (offset < BOOT_SECT_SIZE)
	{
	  /* Can't write boot sector */
		 ...
		 {
		   startedWriting = TRUE; // We are _really_ writing...
