...
6         time mid byte ( (p >> 8) & 0xFF )
7         time low byte ( p & 0xFF )
8		  speed set-point (in rpm) high byte ( v >> 8 ) (setVelocity is of type int16)
9		  speed set-point low byte ( v & 0xFF )
10		  actual speed (in rpm) high byte (velocity is of type int16)
11		  actual speed low byte
12-15     current in Ampere encoded as float32
16        control point high byte (is of type int16)
17        control point low byte
18         position high byte ( (p >> 24) & 0xFF ) (time is of type uint32)
19         position mid byte ( (p >> 16) & 0xFF )
20         position mid byte ( (p >> 8) & 0xFF )
21         position low byte ( p & 0xFF )

Set accel limits enabled
byte			value
0					12
1					0
2					1
