...
extern unsigned int fpga_msecs_motorsStopped;
...
extern void produce_gyro_z_angle( void );
...
#ifdef PRINTF_DEBUG
	printf( "TAG_SET_MOTOR_PID_PARAMETERS %d %d %d %d\n\r", (int)motor, (int)data[2], (int)data[3], (int)data[4] );
#endif

	//motor = motor_mapping[motor];
	decode_float( data+2, &f );
	motor_set_kp( motor, f );

	decode_float( data+6, &f );
	motor_set_ki( motor, f );
}
...
void parse_can_msg( unsigned char* data )
{
	...
	else
	{
		...
		CAN_TxMsg[0].id = id;
		CAN_TxMsg[0].len = dlc;
		memcpy( CAN_TxMsg[0].data, canData, 8 );
...
void parser_gyro_set_param( unsigned char* data )
{
	float value;
	
	if( 5 != data[0] )
	{
		snprintf( com_get_string_buffer(), COM_STR_BUFFER_SIZE, "TAG_GYRO_SET_PARAM invalid length %d", (int)data[0] );
		produce_error();
		return;
	}
	
	decode_float( data+2, &value );
	
	switch( data[1] )
	{
		case GYRO_PARAM_BIAS:
			gyro_set_bias( value );
			break;
		
		case GYRO_PARAM_SCALE:
			gyro_set_scale( value );
			break;
		
		default:
			snprintf( com_get_string_buffer(), COM_STR_BUFFER_SIZE, "TAG_GYRO_SET_PARAM invalid PARAM %d", (int)data[1] );
			produce_error();
			return;
			break;
	}
