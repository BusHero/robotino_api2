...
CAN_msg CAN_charger_info[3];
unsigned int CAN_charger_info_count[3] = {0,0,0};
CAN_msg CAN_charger_event[3];
unsigned int CAN_charger_event_count[3] = {0,0,0};
CAN_msg CAN_charger_version[3];
unsigned int CAN_charger_version_count[3] = {0,0,0};
CAN_msg CAN_charger_error[3];
unsigned int CAN_charger_error_count[3] = {0,0,0};
...
static void CAN_cfgBaudrate (uint32_t ctrl, uint32_t baudrate)  {
  ...
  unsigned long PCLK = (ctrl == 1) ? CAN1_CLK : CAN2_CLK;
  uint32_t result = 0;
...
void CAN_setup (uint32_t ctrl, uint32_t baudrate)  {
  ...
  CAN_cfgBaudrate(ctrl, baudrate);                 /* Set bit timing */
...
void CAN_writeGetVersion( unsigned char chargerID )
{
	int i=10;
	while( 0 == CAN_TxRdy[0] && i > 0 )
	{
		printf("Wait for CAN_Tx 0 Ready\n\r");
		wait_ms( 1 );
		--i;
	}
	...
	if( 0 == CAN_TxRdy[0] )
	{
		printf("CAN 0 not ready\n\r");
		return;
	}

	CAN_TxRdy[0] = 0;

	CAN_TxMsg[0].id = 0x511 + chargerID*0x20;
	...
	CAN_TxMsg[0].data[2] = 0;
	CAN_TxMsg[0].data[3] = 0;
	CAN_TxMsg[0].data[4] = 0;
	CAN_TxMsg[0].data[5] = 0;
	CAN_TxMsg[0].data[6] = 0;
	CAN_TxMsg[0].data[7] = 0;

	CAN_wrMsg( 1, &CAN_TxMsg[0] );
}

void CAN_writeClearError( unsigned char chargerID )
{
	int i=10;
	while( 0 == CAN_TxRdy[0] && i > 0 )
	{
	...
	CAN_TxMsg[0].id = 0x512 + chargerID*0x20;
	CAN_TxMsg[0].format = STANDARD_FORMAT;
	CAN_TxMsg[0].type = DATA_FRAME;

	CAN_TxMsg[0].len = 8;
	CAN_TxMsg[0].data[0] = 0xAA;
	CAN_TxMsg[0].data[1] = 0x55;
	CAN_TxMsg[0].data[2] = 0;
	CAN_TxMsg[0].data[3] = 0;
	CAN_TxMsg[0].data[4] = 0;
	CAN_TxMsg[0].data[5] = 0;
...
static void CAN_ISR (void) __irq  {
  ...
  if (CAN1->GSR & (1 << 0)) {                    /* CAN Controller #1 meassage is received */
	...
	case 0x521:
	case 0x541:
		index=(CAN_RxMsg[0].id-0x501) >> 5;
		CAN_copyMsg( &CAN_charger_info[index], &CAN_RxMsg[0] );
		++CAN_charger_info_count[index];
		break;

	case 0x502:
	case 0x522:
	case 0x542:
		index=(CAN_RxMsg[0].id-0x502) >> 5;
		CAN_copyMsg( &CAN_charger_event[index], &CAN_RxMsg[0] );
		++CAN_charger_event_count[index];
		break;

	case 0x503:
	case 0x523:
	case 0x543:
		index=(CAN_RxMsg[0].id-0x503) >> 5;
		CAN_copyMsg( &CAN_charger_version[index], &CAN_RxMsg[0] );
		++CAN_charger_version_count[index];
		break;

	case 0x504:
	case 0x524:
	case 0x544:
		index=(CAN_RxMsg[0].id-0x504) >> 5;
		CAN_copyMsg( &CAN_charger_error[index], &CAN_RxMsg[0] );
		++CAN_charger_error_count[index];
		break;
