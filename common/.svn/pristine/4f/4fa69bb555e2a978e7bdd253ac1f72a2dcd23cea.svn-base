...
{
	return open( rec::serialport::friendlyName( port ).c_str() );
}
...
bool ComImpl::open( const QString& port )
{
	...
	for( unsigned int speed=0; speed < speedsSize; ++speed)
	{
		...
		try
		{
			...
			_serialPort.open( port.toStdString(), speeds[speed], 1000 );
			...
			{
				std::ostringstream os;
				os << "Failed connecting to " << port.toStdString() << " with speed " << speeds[speed] << " baud";
				reportError( os.str().c_str() );
				continue;
			}
...
{
	QString str = QString::fromStdString( rec::serialport::friendlyName( _serialPort.port() ) );
	return str;
}
...
unsigned int ComImpl::room() const
{
	return _room;
}
...
float ComImpl::ceilingCal() const
{
	return _ceilingCal;
}

float ComImpl::readCeilingCal()
{
	if( !isOpen() )
	{
		return 0.0f;
